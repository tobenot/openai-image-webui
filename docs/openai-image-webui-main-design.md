...
> **这是初版设计稿，保留作为设计脉络的记录，不再同步更新。**
>
> 实际实现已经走得比这份文档远了。主要差异：
>
> - **图生图 / 图片编辑做了**（本文原写"第一版不做"），带 mask 局部重绘
> - **多出三个工作台**：批量生图（prompt 列表）、AI 批量重命名、图片库
> - **图片存 IndexedDB**（本文只提了 localStorage），任务元数据仍在 localStorage
> - **部署路径**改成了自定义域名 + CNAME，不是 `/openai-image-webui/` 子路径
> - 加了成本估算、per-task debug 视图、中英双语
>
> 当前能力以 [`README.md`](../README.md) 为准。下文保持原样。
